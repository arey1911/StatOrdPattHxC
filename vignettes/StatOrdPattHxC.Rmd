...
Currently, it provides the asymptotic variance of the Shannon, Tsallis, and Rényi entropies, and the Fisher information measure.
...
Assume $\mathbf x$ is the time series under analysis.
It has length $n$, and the ordinal patters were computed with embedding dimension $D$.
We obtained the sample Shannon entropy $\widehat{H}(\mathbf x)$ with sample asymptotic variance $s^2(\mathbf x)$.
The asymptotic confidence interval at level $1-\alpha$ is
$$
\bigg[
\widehat{H}(\mathbf x) - z_\alpha \frac{s(\mathbf x)}{\sqrt{n-D}},
\widehat{H}(\mathbf x) + z_\alpha \frac{s(\mathbf x)}{\sqrt{n-D}}
\bigg],
$$
where $z_\alpha$ is the $1-\alpha$ quantile of the standard normal distribution.
The following code illustrates how to compute and display three entropies with their confidence intervals in the $H\times C$ plane.
We include the wite noise time series for illustration purposes only.
...
SemiLength <- StandardDeviations/sqrt(length(wnsub)-D)*qnorm(0.95) 
