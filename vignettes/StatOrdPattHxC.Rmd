...
```{r PointsWithConfidenceIntervals, out.width="100%", echo=FALSE, message=FALSE}
data("LinfLsup")
...
ShannonEntropies <- c(
  HShannon(OPprob(wnsub, emb=4)),
  HShannon(OPprob(x1sub, emb=4)),
  HShannon(OPprob(x2sub, emb=4))
)

StatisticalComplexities <- c(
  StatComplexity(OPprob(wnsub, emb=4)),
  StatComplexity(OPprob(x1sub, emb=4)),
  StatComplexity(OPprob(x2sub, emb=4))
  )

StandardDeviations <- sqrt(Variances[,1])
SemiLength <- StandardDeviations/sqrt(1000-4)*qnorm(0.95)
...
ggplot(subset(LinfLsup, Side=="Lower" & Dimension=="4"), 
       aes(x=H, y=C)) +
  geom_line() +
  geom_line(data=subset(LinfLsup, Side=="Upper" & Dimension=="4"), 
            aes(x=H, y=C)) +
  xlab(expression(italic(H))) +
  ylab(expression(italic(C))) +
